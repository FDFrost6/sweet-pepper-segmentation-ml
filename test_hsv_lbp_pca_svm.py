...
def extract_hsv_histogram(img, h_bins=8, s_bins=4, v_bins=4):
...
def extract_lbp_histogram(img, P=8, R=1):
    ...
    # Uniform LBP for P=8 has 59 patterns (0-58)
...
def extract_features(X):
    feats = []
    for img in X:
        hsv_hist = extract_hsv_histogram(img, h_bins=8, s_bins=4, v_bins=4)
        lbp_hist = extract_lbp_histogram(img, P=8, R=1)
        feats.append(np.concatenate([hsv_hist, lbp_hist]))
...
def main():
    ...
    plot_precision_recall(y_valid, y_valid_scores, title="precision-recall curve (val set, SVM HSV+LBP PCA)")

    # Find threshold for desired recall (e.g., maximize recall for sweet pepper)
    precision, recall, thresholds = precision_recall_curve(y_valid, y_valid_scores)
    # --- FIX: avoid threshold=0.0 if it leads to all-positive predictions ---
    # Instead, pick the threshold that gives the highest F1 or a reasonable tradeoff
...
def main_no_pca():
    ...
    plot_precision_recall(y_valid, y_valid_scores, title="precision-recall curve (val set, SVM HSV+LBP NO PCA)")
